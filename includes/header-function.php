...
<h2 class="desc">
<?php bloginfo('description', 'klasik' ); ?>
</h2>
<?php }else{ ?>
<h1><a href="<?php echo esc_url( home_url( '/' ) );  ?>" title="<?php echo esc_attr( get_bloginfo( 'name', 'display' ) ); ?>"><?php echo $sitename; ?></a> </h1>
<span h2="desc"><?php echo $tagline; ?></h2>
<?php }?>
<?php } else { ?>
<div id="logoimg"> <a href="<?php echo esc_url( home_url( '/' ) );  ?>" title="<?php echo esc_attr( get_bloginfo( 'name', 'display' ) ); ?>"> <img src="<?php echo $logoimage;?>" alt="" /> </a> </div>
<?php } ?>
<?php 
...
if(!function_exists('klasik_page_title')){
	...
	function klasik_page_title(){
		...
		if(is_singular('portfolio') || is_attachment()){
			...
			printf( __( '%s', 'klasik' ), '<span>' . single_cat_title( '', false ) . '<span class=""> Free Porn Videos</span></span>' );
			...
			printf( __( '%s Porn Videos', 'klasik' ), '<span>' . get_search_query() . '</span>' );
			...
			echo 'Welcome! You are browsing the latest free porn videos.';
